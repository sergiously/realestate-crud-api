...
): Promise<{ rows: Post[], count: number }> => await Post.findAndCountAll({
  attributes: { exclude: ['isActive'] },
  where: {
    isActive: true,
    ...(filters?.titleLike && { title: { [Op.like]: `%${filters.titleLike}%` } }),
    ...(filters?.addressLike && { address: { [Op.like]: `%${filters.addressLike}%` } }),
    ...(filters?.minSquareMeters && { squareMeters: { [Op.gte]: filters.minSquareMeters } }),
    ...(filters?.maxSquareMeters && { squareMeters: { [Op.lte]: filters.maxSquareMeters } }),
    ...(filters?.bedrooms && { bedrooms: filters.bedrooms }),
    ...(filters?.minBedrooms && { bedrooms: { [Op.gte]: filters.minBedrooms } }),
    ...(filters?.bathrooms && { bedrooms: filters.bathrooms }),
    ...(filters?.minBathrooms && { bedrooms: { [Op.gte]: filters.minBathrooms } }),
    ...(filters?.minLifeQualityIndex && { lifeQualityIndex: { [Op.gte]: filters.minLifeQualityIndex } }),
    ...(filters?.maxLifeQualityIndex && { lifeQualityIndex: { [Op.lte]: filters.maxLifeQualityIndex } }),
    ...(typeof filters?.hasPorch === 'boolean' && { hasPorch: filters.hasPorch }),
    ...
    ...(filters?.minCreatedAt && { createdAt: { [Op.gte]: filters.minCreatedAt } }),
    ...(filters?.maxCreatedAt && { createdAt: { [Op.lte]: filters.maxCreatedAt } }),
    ...(filters?.minUpdatedAt && { updatedAt: { [Op.gte]: filters.minUpdatedAt } }),
    ...(filters?.maxUpdatedAt && { updatedAt: { [Op.lte]: filters.maxUpdatedAt } }),
 ...
 */
const getByKey = async (id: number): Promise<Post | null> => await Post.findByPk(id, {
  raw: true
});
 ...
 */
const insertWithStatusHistoryAndPriceHistory = async (postRequest: Post): Promise<Post> =>
 ...
 */
const updateWithStatusHistoryAndPriceHistory = async (id: number, postRequest: Post): Promise<boolean> =>
  await sequelize.transaction(async (transaction: Transaction) => {
...
const softDelete = async (id: number): Promise<number> => {
  const result = await Post.update({ isActive: false }, {
    where: {
      id,
      isActive: true,
    },

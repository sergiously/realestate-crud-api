 ...
 */
const search = async (
  query: SearchPostsQueryParams,
): Promise<{ results: Post[]; total: number }> => {
  const { limit, offset, orderBy, orderDirection, ...filters } = query;

  const posts = await PostDal.getAllActiveWithStatusAndPrice(
    filters,
    orderBy,
    orderDirection,
    limit,
 ...
 */
const create = async (postRequest: Post): Promise<Post | null> =>
  await PostDal.insertWithStatusHistoryAndPriceHistory(postRequest);
 ...
 */
const update = async (id: number, postRequest: Post): Promise<boolean> =>
  await PostDal.updateWithStatusHistoryAndPriceHistory(id, postRequest);
